...
public:
	...
	void DrawTex(const ITexture* tex, int x, int y, int w, int h, const Math::Rectangle* srcRect = nullptr, const Math::Color* color = &kWhiteColor, const Math::Vector2& pivot = { 0.5, 0.5f });
	void DrawTex(const std::weak_ptr<ITexture> tex, int x, int y, int w, int h, const Math::Rectangle* srcRect = nullptr, const Math::Color* color = &kWhiteColor, const Math::Vector2& pivot = { 0.5, 0.5f })
	{
	...
	// ・color			… 色(RGBA)
	// ・pivot			… 基準点 0.0～1.0の範囲で指定する
	void DrawTex(const ITexture* tex, int x, int y, const Math::Rectangle* srcRect = nullptr, const Math::Color* color = &kWhiteColor, const Math::Vector2& pivot = { 0.5, 0.5f })
	{
		if (tex == nullptr)return;
		DrawTex(tex, x, y, tex->GetInfo()->Width, tex->GetInfo()->Height, srcRect, color, pivot);
	}
	void DrawTex(const std::weak_ptr<ITexture> tex, int x, int y, const Math::Rectangle* srcRect = nullptr, const Math::Color* color = &kWhiteColor, const Math::Vector2& pivot = { 0.5, 0.5f })
	{
		if (tex.expired())return;
