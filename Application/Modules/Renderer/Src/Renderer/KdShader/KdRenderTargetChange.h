...
struct KdRenderTargetPack
{
	...
	void SetRenderTarget(std::shared_ptr<ITexture> RTT, std::shared_ptr<ITexture> DST = nullptr, D3D11_VIEWPORT* pVP = nullptr);

	void SetViewPort(D3D11_VIEWPORT* pVP);

	void ClearTexture(const DirectX::SimpleMath::Color& fillColor = {1,1,1,1});

	std::shared_ptr<ITexture> m_RTTexture = nullptr;
	std::shared_ptr<ITexture> m_ZBuffer = nullptr;
...
struct KdRenderTargetChanger
{
	...
	bool ChangeRenderTarget(std::shared_ptr<ITexture> RTT, std::shared_ptr<ITexture> DST = nullptr, D3D11_VIEWPORT* pVP = nullptr);
