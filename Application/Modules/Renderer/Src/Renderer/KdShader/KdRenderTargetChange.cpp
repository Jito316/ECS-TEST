...
void KdRenderTargetPack::SetRenderTarget(std::shared_ptr<ITexture> RTT, std::shared_ptr<ITexture> DST, D3D11_VIEWPORT* pVP)
{
	m_RTTexture = RTT;
	m_ZBuffer = DST;
...
void KdRenderTargetPack::ClearTexture(const DirectX::SimpleMath::Color& fillColor)
{
	ID3D11DeviceContext* DevCon = GETRENDERER->m_kdDirect3D->WorkDevContext();
...
bool KdRenderTargetChanger::ChangeRenderTarget(std::shared_ptr<ITexture> RTT,
	std::shared_ptr<ITexture> DST, D3D11_VIEWPORT* pVP)
{
	if (!RTT) { return false; }
...
void KdRenderTargetChanger::UndoRenderTarget()
{
	...
	GETRENDERER->m_kdDirect3D->WorkDevContext()->OMSetRenderTargets(1, &m_saveRTV, m_saveDSV);

	if (m_changeVP)
	{
		GETRENDERER->m_kdDirect3D->WorkDevContext()->RSSetViewports(1, &m_saveVP);
	}
