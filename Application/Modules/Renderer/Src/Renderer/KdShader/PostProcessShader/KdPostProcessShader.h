...
public:
	...
	void GenerateBlurTexture(std::shared_ptr<ITexture>& spSrcTex, std::shared_ptr<ITexture>& spDstTex, D3D11_VIEWPORT& VP, int blurRadius);
...
private:
	...
	void CreateBlurOffsetList(std::vector<Math::Vector3>& dstInfo, const std::shared_ptr<ITexture>& spSrcTex, int samplingSize, const Math::Vector2& dir);

	void DrawTexture(std::shared_ptr<ITexture>* spSrcTex, int srcTexSize, std::shared_ptr<ITexture> spDstTex, D3D11_VIEWPORT* pVP);

	void SetBlurInfo(const std::shared_ptr<ITexture>& spSrcTex, int samplingSize, const Math::Vector2& dir);
