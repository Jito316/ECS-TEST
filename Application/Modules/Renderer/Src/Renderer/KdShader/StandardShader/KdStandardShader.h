...
#include "Renderer/KdShader/KdRenderTargetChange.h"
...
public:
	...
	// ディゾルブテクスチャ設定
	void SetDissolveTexture(ITexture& dissolveMask);

	// デフォルトディゾルブテクスチャ設定
	void SetDefaultDissolveTexture(std::shared_ptr<ITexture>& spDissolveMask)
	{
	...
	//================================================
	// メッシュ描画
	void DrawMesh(const IMesh* mesh, const Math::Matrix& mWorld, const std::vector<std::shared_ptr<IMaterial>>& materials,
		const Math::Vector4& col, const Math::Vector3& emissive);

	// モデルデータ描画：アニメーションに非対応
	void DrawModel(const IModelData& rModel, const Math::Matrix& mWorld = Math::Matrix::Identity,
		const Math::Color& colRate = kWhiteColor, const Math::Vector3& emissive = Math::Vector3::Zero);

	// モデルワーク描画：アニメーションに対応
	void DrawModel(IModelWork& rModel, const Math::Matrix& mWorld = Math::Matrix::Identity,
		const Math::Color& colRate = kWhiteColor, const Math::Vector3& emissive = Math::Vector3::Zero);

	// 任意の頂点群からなるポリゴン描画
	void DrawPolygon(const IPolygon& poly, const Math::Matrix& mWorld = Math::Matrix::Identity,
		const Math::Color& colRate = kWhiteColor, const Math::Vector3& emissive = Math::Vector3::Zero);

	// 任意の頂点群からなるポリゴンライン描画
	void DrawVertices(const std::vector<IPolygon::Vertex>& vertices, const Math::Matrix& mWorld = Math::Matrix::Identity,
	...
	std::shared_ptr<ITexture>& GetDepthTex() { return m_depthMapFromLightRTPack.m_RTTexture; }

private:

	// マテリアルのセット
	void WriteMaterial(const IMaterial& material, const Math::Vector4& colRate, const Math::Vector3& emiRate);

	// ポリゴンの法線情報を2Dように書き換える
	void ConvertNormalsFor2D(std::vector<IPolygon::Vertex>& target, const Math::Matrix& mWorld);
	...
	// テクスチャ
	std::shared_ptr<ITexture>	m_dissolveTex = nullptr;	// ディゾルブで使用するデフォルトテクスチャ
