...
void KdModelData::CreateNodes(const std::shared_ptr<KdGLTFModel>& spGltfModel)
{
	...
	for (UINT i = 0; i < spGltfModel->Nodes.size(); i++)
	{
		...
		{
			// メッシュ作成
			auto mesh = std::make_shared<KdMesh>();

			if (mesh)
			{
				mesh->Create(rSrcNode.Mesh.Vertices, rSrcNode.Mesh.Faces, rSrcNode.Mesh.Subsets, rSrcNode.Mesh.IsSkinMesh);
			}
			rDstNode.m_spMesh = mesh;
...
void KdModelData::CreateMaterials(const std::shared_ptr<KdGLTFModel>& spGltfModel, const std::string& fileDir)
{
	...
	for (UINT i = 0; i < m_materials.size(); ++i)
	{
		...
		IMaterial& rDstMaterial = *(m_materials[i] = std::make_shared<KdMaterial>());

		// 名前
		rDstMaterial.SetName(rSrcMaterial.Name);
		...
		// 基本色
		rDstMaterial.SetBaseColorRate(rSrcMaterial.BaseColor);

		// 金属性・粗さ
		rDstMaterial.SetMetallicRate(rSrcMaterial.Metallic);
		rDstMaterial.SetRoughnessRate(rSrcMaterial.Roughness);

		// 自己発光・エミッシブ
		rDstMaterial.SetEmissiveRate(rSrcMaterial.Emissive);
	}
}
...
void KdModelData::CreateAnimations(const std::shared_ptr<KdGLTFModel>& spGltfModel)
{
	...
	for (UINT i = 0; i < m_spAnimations.size(); ++i)
	{
		...
		m_spAnimations[i] = std::make_shared<JTN::Math::KdAnimationData>();
		JTN::Math::KdAnimationData& rDstAnimation = *(m_spAnimations[i]);
...
// アニメーションデータ取得：文字列検索
const std::shared_ptr<JTN::Math::KdAnimationData> KdModelData::GetAnimation(std::string_view animName) const
{
...
// アニメーションデータ取得：番号指定
const std::shared_ptr<JTN::Math::KdAnimationData> KdModelData::GetAnimation(UINT index) const
{
	return index >= m_spAnimations.size() ? nullptr : m_spAnimations[index];
}

const std::vector<std::shared_ptr<JTN::Math::KdAnimationData>> KdModelData::GetAnimations() const
{
...
// モデル設定：コピーノードの生成
void KdModelWork::SetModelData(const std::shared_ptr<IModelData>& rModel)
{ 
	m_spData = rModel;
...
{
	// モデルのセット
	SetModelData(GETRENDERER->m_kdAssets->m_modeldatas.GetData(fileName));
}
