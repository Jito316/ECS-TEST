...
static bool KdCreateViewsFromTexture2D(ID3D11Texture2D* resource, ID3D11ShaderResourceView** ppSRV, ID3D11RenderTargetView** ppRTV, ID3D11DepthStencilView** ppDSV)
{
	...
	if (ppRTV && desc.BindFlags & D3D11_BIND_RENDER_TARGET)
	{
		...
		HRESULT hr = GETMODULE(IRenderer)->GetDirect3D()->WorkDev()->CreateRenderTargetView(resource, &rtvDesc, ppRTV);
		if (FAILED(hr))
		{
	...
	if (ppSRV && desc.BindFlags & D3D11_BIND_SHADER_RESOURCE)
	{
		...
		// シェーダリソースビュー作成
		HRESULT hr = GETRENDERER->m_kdDirect3D->WorkDev()->CreateShaderResourceView(resource, &srvDesc, ppSRV);
		if (FAILED(hr))
		{
	...
	if (ppDSV && desc.BindFlags & D3D11_BIND_DEPTH_STENCIL) {
		...
		// デプスステンシルビュー作成
		//-------------------------------
		HRESULT hr = GETRENDERER->m_kdDirect3D->WorkDev()->CreateDepthStencilView(resource, &dsvDesc, ppDSV);
		if (FAILED(hr)) {
...
bool KdTexture::Load(std::string_view filename, bool renderTarget, bool depthStencil, bool generateMipmap)
{
	...
	if (FAILED(DirectX::CreateTextureEx(
		GETRENDERER->m_kdDirect3D->WorkDev(),				// Direct3D Device
		image.GetImages(),
		image.GetImageCount(),
		image.GetMetadata(),
...
bool KdTexture::Create(const D3D11_TEXTURE2D_DESC & desc, const D3D11_SUBRESOURCE_DATA * fillData)
{
	...
	HRESULT hr = GETRENDERER->m_kdDirect3D->WorkDev()->CreateTexture2D(&desc, fillData, &resource);
	if (FAILED(hr)) {
		Release();
		return false;
	}
