...
bool ConvertRectToUV(const ITexture* srcTex, const Math::Rectangle& src, Math::Vector2& uvMin, Math::Vector2& uvMax)
{
...
void KdSquarePolygon::SetUVRect(const Math::Rectangle& _rect)
{
	...
	Math::Vector2 uvMin, uvMax;
	ConvertRectToUV(m_spMaterial->GetBaseColorTex().get(), _rect, uvMin, uvMax);
