...
KdCamera::~KdCamera()
{
	...
	if (render.GetMainCamera())
	{
		render.SetMainCamera(nullptr);
	}
	auto it = std::ranges::find(render.GetSubCameraList(), this);
	if (it != render.GetSubCameraList().end())
	{
		render.GetSubCameraList().erase(it);
	}
}
...
void KdCamera::SetToShader() const
{
	...
	// カメラの情報をGPUへ転送
	render->m_kdShaderManager->WriteCBCamera(m_mCam, m_mProj);
	...
	// フォーカスを合わせる焦点距離をコピー
	render->m_kdShaderManager->m_postProcessShader.SetNearClippingDistance(nearClippingDistance);
	render->m_kdShaderManager->m_postProcessShader.SetFarClippingDistance(farClippingDistance);
	render->m_kdShaderManager->m_postProcessShader.SetFocusDistance((m_focusDistance - nearClippingDistance) / viewRange);
	render->m_kdShaderManager->m_postProcessShader.SetFocusRange(m_focusForeRange / viewRange, m_focusBackRange / viewRange);
}
...
void KdCamera::SetProjectionMatrix(float fov, float maxRange, float minRange, float aspectRatio)
{
	...
	if (aspect <= 0)
	{
		...
		if (GETRENDERER->m_kdDirect3D->GetBackBuffer())
		{
			aspect = GETRENDERER->m_kdDirect3D->GetBackBuffer()->GetAspectRatio();
		}
...
void KdCamera::GenerateRayInfoFromClientPos(const POINT& clientPos, Math::Vector3& rayPos, Math::Vector3& rayDir, float& rayRange)
{
	...
	GETRENDERER->m_kdDirect3D->ClientToWorld(clientPos, 0.0f, rayPos, m_mCam, m_mProj);
	GETRENDERER->m_kdDirect3D->ClientToWorld(clientPos, 1.0f, farPos, m_mCam, m_mProj);
...
void KdCamera::ConvertWorldToScreenDetail(const Math::Vector3& pos, Math::Vector3& result)
{
	// ビューポートを取得する
	Math::Viewport vp;
	GETRENDERER->m_kdDirect3D->CopyViewportInfo(vp);
...
void KdCamera::Register(Type _type) const
{
	switch (_type) {
	case Type::Main:
		GETRENDERER->SetMainCamera((ICamera*)this);
		break;
	case Type::Sub:
		GETRENDERER->SetSubCamera((ICamera*)this);
		break;
	}
}
