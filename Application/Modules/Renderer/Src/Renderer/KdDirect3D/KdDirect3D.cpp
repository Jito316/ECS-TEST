...
bool KdDirect3D::Init(HWND hWnd, int w, int h, bool deviceDebug, std::string& errMsg)
{
	...
	*/
	...
	// ブレンドステートセット
	ID3D11BlendState* bs = CreateBlendState(
		KdBlendMode::Alpha);
...
ID3D11RasterizerState* KdDirect3D::CreateRasterizerState(int cullMode, int fillMode, bool depthClipEnable, bool scissorEnable) const
{
	// ラスタライズの動作設定
...
ID3D11SamplerState* KdDirect3D::CreateSamplerState(KdSamplerFilterMode filterType, UINT maxAnisotropy
	, KdSamplerAddressingMode addressingMode, bool comparisonModel) const
{
...
void KdDirect3D::DrawVertices(int topology, int vertexCount, const void* pVertexStream, UINT stride)
{
	assert(m_pDeviceContext && "デバイスコンテキストが見つかりません");

	// プリミティブトポロジーをセット
	m_pDeviceContext->IASetPrimitiveTopology(static_cast<D3D_PRIMITIVE_TOPOLOGY>(topology));
