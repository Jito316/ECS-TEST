...
void KdMesh::SetToDevice() const
{
	...
	UINT offset = 0;					// オフセット
	GETRENDERER->m_kdDirect3D->WorkDevContext()->IASetVertexBuffers(0, 1, m_vertBuf.GetAddress(), &stride, &offset);

	// インデックスバッファセット
	GETRENDERER->m_kdDirect3D->WorkDevContext()->IASetIndexBuffer(m_indxBuf.GetBuffer(), DXGI_FORMAT_R32_UINT, 0);

	//プリミティブ・トポロジーをセット
	GETRENDERER->m_kdDirect3D->WorkDevContext()->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
}
...
void KdMesh::DrawSubset(int subsetNo) const
{
	...
	// 描画
	GETRENDERER->m_kdDirect3D->WorkDevContext()->DrawIndexed(m_subsets[subsetNo].FaceCount * 3, m_subsets[subsetNo].FaceStart * 3, 0);
}
