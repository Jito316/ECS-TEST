...
struct KdGLTFAnimationData
{
	...
	struct Node
	{
		...
		// 各チャンネル
		std::vector<Math::KdAnimKeyVector3>		m_translations;	// 位置キーリスト
		std::vector<Math::KdAnimKeyQuaternion>	m_rotations;	// 回転キーリスト
		std::vector<Math::KdAnimKeyVector3>		m_scales;		// 拡大キーリスト
	};
	// 全ノード用アニメーションデータ
