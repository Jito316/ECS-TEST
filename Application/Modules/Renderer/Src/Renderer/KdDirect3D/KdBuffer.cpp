...
bool KdBuffer::Create(UINT bindFlags, UINT bufferSize, D3D11_USAGE bufferUsage, const D3D11_SUBRESOURCE_DATA* initData)
{
	...
	//----------------------------------
	if ((D3D11_BIND_FLAG)bindFlags == D3D11_BIND_CONSTANT_BUFFER)
	{
	...
	// バッファ作成
	//--------------------------------
	if (FAILED(GETRENDERER->m_kdDirect3D->WorkDev()->CreateBuffer(&desc, initData, &m_pBuffer)))
	{
...
void KdBuffer::WriteData(const void* pSrcData, UINT size)
{
	...
	if (m_bufUsage == D3D11_USAGE_DYNAMIC)
	{
		...
		// 書き込み専用
		if (SUCCEEDED(GETRENDERER->m_kdDirect3D->WorkDevContext()->Map(m_pBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &pData)))
		{
			memcpy_s(pData.pData, m_bufSize, pSrcData, size);

			GETRENDERER->m_kdDirect3D->WorkDevContext()->Unmap(m_pBuffer, 0);
		}
	}
	// 静的バッファの場合
	...
	{
		// バッファに書き込み
		GETRENDERER->m_kdDirect3D->WorkDevContext()->UpdateSubresource(m_pBuffer, 0, 0, pSrcData, 0, 0);
	}
	// ステージングバッファの場合
	else if (m_bufUsage == D3D11_USAGE_STAGING)
	{
		// 読み書き両方
		D3D11_MAPPED_SUBRESOURCE pData;
		if (SUCCEEDED(GETRENDERER->m_kdDirect3D->WorkDevContext()->Map(m_pBuffer, 0, D3D11_MAP_READ_WRITE, 0, &pData)))
		{
			memcpy_s(pData.pData, m_bufSize, pSrcData, size);
...
void KdBuffer::CopyFrom(const KdBuffer& srcBuffer)
{
	...
	GETRENDERER->m_kdDirect3D->WorkDevContext()->CopyResource(m_pBuffer, srcBuffer.GetBuffer());
}
