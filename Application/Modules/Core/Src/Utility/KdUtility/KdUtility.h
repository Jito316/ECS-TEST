...
//===========================================
// 算術系短縮名
namespace Math = JTN::Math;

// 角度変換
constexpr float KdPI = 3.141592654f;
constexpr float KdToRadians = (KdPI / 180.0f);
constexpr float KdToDegrees = (180.0f / KdPI);
...
CORE_API inline bool KdFileExistence(std::string_view path)
{
...
CORE_API inline std::string KdGetDirFromPath(const std::string &path)
{
...
CORE_API inline std::string KdGetNameFromPath(const std::string& path, bool onlyFileName = false)
{
...
CORE_API inline std::string KdFormat(std::string_view fmt, Args ... args)
{
...
//===========================================
/*
namespace JTN { namespace Module { namespace Renderer { namespace D3D11 { class ITexture; } } } }
CORE_API bool ConvertRectToUV(const JTN::Module::Renderer::D3D11::ITexture* srcTex, const Math::Rectangle& src, Math::Vector2& uvMin, Math::Vector2& uvMax);
*/
...
//
//===========================================
CORE_API float EaseInOutSine(float progress);

CORE_API inline JTN::Math::Vector3 ConvertToRadian(const JTN::Math::Vector3& _degree)
{
	JTN::Math::Vector3 vec3;
	vec3.x = _degree.x * KdToRadians;
	vec3.y = _degree.y * KdToRadians;
	vec3.z = _degree.z * KdToRadians;
	return vec3;
}
