	...
	namespace Math {
		...
		public:
				...
				:UINT
			{
				...
				TypeBound = 1 << 6,   // 跳ね返り : 相手を跳ね飛ばすオブジェクトに付与する
				TypeBlock = 1 << 7,
				TypeScene = 1 << 8
			};
			...
			struct CollisionResult
			{
				...
				//後付け
				Math::Vector3 m_objectPos;      //当たったオブジェクトの座標
				UINT m_type;
			};
			...
			// 当たり判定形状形状登録
			void RegisterCollisionShape(std::string_view name, std::shared_ptr<class KdCollisionShape> spShape);
		...
		private:
			struct ColliderPimple;
			ColliderPimple* m_colliderPimple = nullptr;
		};
		...
		// ///// ///// ///// ///// ///// ///// ///// ///// ///// ///// ///// ///// ///// ///// /////
		class CORE_API KdCollisionShape
		{
		public:
		...
		public:
			...
			KdBoxCollision(UINT type, const Math::Matrix& matrix, const Math::Vector3& offset, const Math::Vector3& size, const bool isOriented) :
				KdCollisionShape(type), m_isOriented(isOriented) {
				if (!isOriented)
				{
					m_Abox.Center = matrix.Translation() + offset;
					m_Abox.Extents = size;
